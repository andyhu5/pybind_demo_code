...
float my_add(float arg1, float arg2) {
  return arg1 + arg2;
}

std::vector<double> Foo(const std::vector<double>& input) {
...
py::tuple Bar(const py::tuple& input) {
    std::vector<double> result;
    result.reserve(input.size()); // 预分配空间以提高性能
    for (auto item : input) {
        result.push_back(item.cast<double>() * 2.0);
    }
    return py::make_tuple(result); // 将 std::vector 转换为 Python tuple
}
...
PYBIND11_MODULE(module_name, module_handle) {
  ...
  module_handle.def("my_add", &my_add, "A function that adds two numbers");
  module_handle.def("some_class_factory", &some_class_factory);
  module_handle.def("Foo", &Foo, "A function that doubles each element in a vector");
  module_handle.def("Bar", &Bar, "A function that doubles each element in a tuple");
  py::class_<SomeClass>(
